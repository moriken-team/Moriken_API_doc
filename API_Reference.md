...
|:------------:|:----------|:---|:----------:|
|kentei_id|どの検定の回答履歴か(1:もりけんweb,2:iOSapp,3:Androidapp,4:ガンライザー検定,5:たきざわ検定web,6:たきざわ検定app)|int|◯|
|employ|年度(0:オリジナル問題,0以外:過去問対象西暦)|int|◯<br>※1は例外|
|public_flag|問題の公開フラグ(非公開:0,公開:1)|int|オリジナル問題抽出時は常に◯|
|user_id|問題作成者ID|int|オリジナル問題抽出時は常に◯|
|category_id|カテゴリID|int|級かカテゴリIDどちらか一つが◯<br />(過去問抽出時)<br>※1は例外|
|grade|級<br>たきざわ検定app => 初年度(級なし):0，次年度:初級が3,中級が2|int|級かカテゴリIDどちらか一つが◯<br />(過去問抽出時のみ)<br>※1は例外|
|item|問題取得数|int|◯|
※1年度，カテゴリID，級が全て入力されない場合はランダム取得とする
